...
| Agent | Pi 拥有 Session / Compaction / Tool Loop。新对话可选 DSH（工作树钉 `0.1.6-alpha.1`）；默认运行时是设置项。子 Agent 默认主工作区。DSH Multitask 用 ACP 子会话并行；模型自己拉起的 DSH `subagent` 也投影到 Working 胶囊。Pi 子代理阻塞。Working 是投影。产品回归 `npm run test:product-loop`。不扫描用户句子做意图路由。 |
